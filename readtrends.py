...
query = ("SELECT keyword.name as name,keyword.id as id,"
         ...
         " where name<>'' and keyword.id=21562 "
...
while not end:
    ...
    try:
        if(termob["name"].endswith("-")):
            termob["name"] = termob["name"][:-1]
        pytrends = TrendReq(hl='pt-Br', tz=360, timeout=(60, 120), retries=0, backoff_factor=0.1,
                                proxies=[proxy[proxyindex]], requests_args={'verify':False})
